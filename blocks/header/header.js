...
export default async function decorate(block) {
  ...
  const navPath = navMeta ? new URL(navMeta, window.location).pathname : (() => { const parts = window.location.pathname.split('/').filter(Boolean); return parts.length > 0 ? `/${parts[0]}/nav` : '/nav'; })();
